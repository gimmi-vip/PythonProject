...
def booking():
    b_frame = Frame(root, height=530, width=1200, bg='skyblue')
    '''path = "newbg6lf.jpg"
    img = ImageTk.PhotoImage(Image.open(path))
...
def customer():
    global rot
    global delentry
    rot = Frame(root,height=530,width=1200)
    rot.config(bg="skyblue")
...
def detailss():
    ...
    cur.execute("select * FROM foodbill WHERE roomno=?", (SEARCH,))
    for x in cur.fetchall():
        fody = int(str(x[1]))
        print(fody)
        # print (x)
...
def checkout():
    ...
    chklist = Listbox(b_frame, height=30, width=70,bg="skyblue",fg="red")
    ...
    Label(b_frame, text="* Grand Total ", bg='skyblue').place(x=500, y=150)
    totalpay = Entry(b_frame, width=15, bd=5)
    totalpay.place(x=600, y=150)
    ...
    Button(b_frame,text="PAY",bg="red",bd=1,fg="white",command=ppp).place(x=700,y=150)
    Button(b_frame, text="Checkout", bg="red", bd=5,relief="groove", fg="white",command=unreserve).place(x=700, y=250)
